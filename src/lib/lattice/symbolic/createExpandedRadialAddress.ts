 ...
 *
 * `normalizedRatio` prserves whether the original ratio lies above or below unison while placeing it in the corresponding target octave.
...
export type ExpandedRadialAddress = Readonly<{
  ...
  side: "upper" | "lower";
  path: ReturnType<typeof createRadialAddress>["path"];
  distance: number;
}>;
 ...
 *
 * The ratio is first normalized into [1, 2) when it is at or above unison, or [1/2, 1) when it is below unison. Powers of 2 do not contribute to the prime-factor path, so normalization preserves the syjmbolic generator structure while retaining the ratio's side of unison.
 *
