...
describe("createLocalFrame", () => {
  it("uses the anchor direction as the local z-axis", () => {
    expect(
      createLocalFrame({
        x: 0,
        y: 0,
  ...
  it("normalizes the anchor direction as the local z-axis", () => {
    const frame = createLocalFrame({ x: 2, y: 0, z: 0 });
  ...
  it("creates an x-axis perpendicular to the local z-axis", () => {
    const frame = createLocalFrame({ x: 1, y: 1, z: 0 });

    expect(frame.origin).toEqual({ x: 1, y: 1, z: 0 });
    expect(dotProduct(frame.xAxis, frame.zAxis)).toBeCloseTo(0);
  });

  it("creates a y-axis perpendicular to both local x and z axes", () => {
    const frame = createLocalFrame({ x: 1, y: 1, z: 1 });

    expect(dotProduct(frame.yAxis, frame.xAxis)).toBeCloseTo(0);
    expect(dotProduct(frame.yAxis, frame.zAxis)).toBeCloseTo(0);
  });

  it("creates a valid frame when the anchor direction is parallel to global up", () => {
    const frame = createLocalFrame({ x: 0, y: 2, z: 0 });
  ...
  it("creates a valid frame when the anchor direction is nearly parallel to global up", () => {
    const frame = createLocalFrame({ x: 0.000001, y: 1, z: 0 });
